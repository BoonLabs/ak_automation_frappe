...
from ak_automation.dispatcher.formula import (
	evaluate_expression,
	evaluate_function,
	resolve_value,
	_eval_conditional,
...
class TestEvaluateExpression(UnitTestCase):
	...
	def test_field_reference_division(self):
		doc = frappe._dict({"annual_revenue": 120000})
...
class TestEvalConditional(UnitTestCase):
	...
	def test_function_in_then_clause(self):
		doc = frappe._dict({"first": "Alice", "last": "Smith"})
...
class TestBuildContext(UnitTestCase):
	...
	def test_doc_fields_as_top_level(self):
		doc = frappe._dict({"status": "Open", "priority": "High"})

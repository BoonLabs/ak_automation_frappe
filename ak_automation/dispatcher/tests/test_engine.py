...
from ak_automation.dispatcher.engine import (
	EVENT_MAP,
	_log_execution,
	_run_automation,
	cleanup_old_logs,
...
class TestHandleEvent(UnitTestCase):
	...
	def test_skips_ak_automation_doctype(self):
		...
		with patch("ak_automation.dispatcher.engine.get_active_automations") as mock_get:
	...
	def test_after_insert_triggers_both_create_and_update(self):
		...
		with patch("ak_automation.dispatcher.engine.get_active_automations", side_effect=mock_get):
	...
	def test_exception_in_automation_does_not_stop_others(self):
		...
		with (
			patch("ak_automation.dispatcher.engine.get_active_automations", return_value=[auto1, auto2]),
			patch("ak_automation.dispatcher.engine._run_automation", side_effect=mock_run),
			patch("ak_automation.dispatcher.engine._log_execution"),
	...
	def test_exception_logs_failure_and_updates_last_error(self):
		...
		with (
			patch("ak_automation.dispatcher.engine.get_active_automations", return_value=[auto]),
			patch("ak_automation.dispatcher.engine._run_automation", side_effect=Exception("test error")),
			patch("ak_automation.dispatcher.engine._log_execution") as mock_log,
	...
	def test_recursion_flag_cleared_after_execution(self):
		...
		with (
			patch("ak_automation.dispatcher.engine.get_active_automations", return_value=[auto]),
			patch("ak_automation.dispatcher.engine._run_automation"),
		):
			handle_event(doc, "on_update")
	...
	def test_recursion_flag_cleared_after_exception(self):
		...
		with (
			patch("ak_automation.dispatcher.engine.get_active_automations", return_value=[auto]),
			patch("ak_automation.dispatcher.engine._run_automation", side_effect=Exception("boom")),
			patch("ak_automation.dispatcher.engine._log_execution"),
			patch("frappe.db.set_value"),
		):
			handle_event(doc, "on_update")
...
class TestRunAutomation(UnitTestCase):
	...
	def test_trigger_field_unchanged_skips(self):
		...
		with (
			patch("ak_automation.dispatcher.engine.evaluate_conditions") as mock_eval,
			patch("ak_automation.dispatcher.engine._log_execution"),
		):
	...
	def test_trigger_field_changed_proceeds(self):
		...
		with (
			patch("ak_automation.dispatcher.engine.evaluate_conditions", return_value=False),
			patch("ak_automation.dispatcher.engine._log_execution"),
		):
			_run_automation(auto, doc, "On Update (includes Create)")
	...
	def test_conditions_pass_executes_actions(self):
		...
		with (
			patch("ak_automation.dispatcher.engine.evaluate_conditions", return_value=True),
			patch("ak_automation.dispatcher.actions.execute_action", return_value="done"),
			patch("ak_automation.dispatcher.engine._log_execution") as mock_log,
			patch("frappe.db.set_value"),
		):
			_run_automation(auto, doc, "On Create")
	...
	def test_disabled_actions_skipped(self):
		...
		with (
			patch("ak_automation.dispatcher.engine.evaluate_conditions", return_value=True),
			patch("ak_automation.dispatcher.actions.execute_action", side_effect=mock_execute),
			patch("ak_automation.dispatcher.engine._log_execution"),
			patch("frappe.db.set_value"),
		):
			_run_automation(auto, doc, "On Create")
	...
	def test_action_failure_raises_and_records(self):
		...
		with (
			patch("ak_automation.dispatcher.engine.evaluate_conditions", return_value=True),
			patch("ak_automation.dispatcher.actions.execute_action", side_effect=Exception("action failed")),
		):
	...
	def test_action_results_recorded(self):
		...
		with (
			patch("ak_automation.dispatcher.engine.evaluate_conditions", return_value=True),
			patch("ak_automation.dispatcher.actions.execute_action", return_value="Updated fields: status"),
			patch("ak_automation.dispatcher.engine._log_execution") as mock_log,
			patch("frappe.db.set_value"),
		):
			_run_automation(auto, doc, "On Create")
...
class TestCronAutomations(UnitTestCase):
	...
	def test_run_cron_automations_skips_non_matching(self):
		...
		with (
			...
			patch("ak_automation.dispatcher.engine._run_cron_automation") as mock_run,
			patch("frappe.log_error"),
		):
			from ak_automation.dispatcher.engine import run_cron_automations

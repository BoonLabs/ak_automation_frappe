...
from ak_automation.dispatcher.actions import execute_action, ACTION_MODULE_MAP
...
class TestExecuteActionDispatch(UnitTestCase):
	...
	def test_routes_to_correct_module(self):
		...
		with patch("ak_automation.dispatcher.actions.run_script.execute", return_value="Script executed") as mock_exec:
...
class TestUpdateFieldsAction(UnitTestCase):
	...
	def test_static_value_update(self):
		...
		with patch("frappe.db.set_value"):
			from ak_automation.dispatcher.actions.update_fields import execute
...
class TestCreateRecordAction(UnitTestCase):
	...
	def test_creates_doc_with_dict_values(self):
		...
		from ak_automation.dispatcher.actions.create_record import execute
		result = execute(action, doc, auto)
...
class TestCreateTodoAction(UnitTestCase):
	...
	def test_creates_todo_with_defaults(self):
		...
		from ak_automation.dispatcher.actions.create_todo import execute
		result = execute(action, doc, auto)
...
class TestCreateEventAction(UnitTestCase):
	...
	def test_creates_event_with_defaults(self):
		...
		from ak_automation.dispatcher.actions.create_event import execute
		result = execute(action, doc, auto)
...
class TestHTTPRequestAction(UnitTestCase):
	...
	def test_get_request(self):
		...
		with patch("ak_automation.dispatcher.actions.http_request.requests.request", return_value=mock_resp) as mock_req:
			from ak_automation.dispatcher.actions.http_request import execute
			result = execute(action, doc, auto)
...
class TestRunScriptAction(UnitTestCase):
	...
	def test_empty_script_returns_message(self):
		...
		from ak_automation.dispatcher.actions.run_script import execute
		result = execute(action, doc, auto)
...
class TestSendEmailAction(UnitTestCase):
	...
	def test_sends_email_with_basic_fields(self):
		...
		with patch("frappe.sendmail") as mock_send:
			from ak_automation.dispatcher.actions.send_email import execute
			result = execute(action, doc, auto)
	...
	def test_newline_separated_emails(self):
		...
		from ak_automation.dispatcher.actions.send_email import _split_emails
...
class TestSendWhatsAppAction(UnitTestCase):
	...
	def test_no_provider_throws(self):
		...
		with patch("frappe.get_single") as mock_settings:
			...
			from ak_automation.dispatcher.actions.send_whatsapp import execute
			with self.assertRaises(Exception):
				execute(action, doc, auto)
	...
	def test_phone_normalization(self):
		...
		from ak_automation.dispatcher.actions.send_whatsapp import _normalize_phone
	...
	def test_meta_text_message(self):
		...
		with (
			...
			patch("ak_automation.dispatcher.actions.send_whatsapp.requests.post", return_value=mock_resp) as mock_post,
		):
			from ak_automation.dispatcher.actions.send_whatsapp import execute
			result = execute(action, doc, auto)
